...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void sendWearableData() {
        ...
        if (cursor.moveToFirst()) {
            ...
            putDataMapReq.getDataMap().putString("MAXTEMP", new StringBuilder().append(Math.round(high)).append("°").toString());
            putDataMapReq.getDataMap().putString("MINTEMP", new StringBuilder().append(Math.round(low)).append("°").toString());
